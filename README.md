...
2. Current Qt scope:
    ...
    - import folder & background thumbnailing: `Import Folder` (Ctrl+I) opens a dialog to select a folder and name a collection (or merge into an existing one); imports queue thumbnails for background processing (one every 500ms so resources don't get hogged), with thumbnails cached to disk under `collections/thumbnails/` so re-selecting a collection or restarting the app doesn't re-decode RAW files. Status bar shows "Importing: 12/47 thumbnails..." as it processes. User can continue editing while import runs in background
    - collections: imports are persisted as named collections (`collections/collections.json`); the filmstrip header's `Collection` dropdown switches the active collection and its images. `Rename`/`Delete` manage a collection, right-click a filmstrip thumbnail to remove just that image from the collection (files on disk are untouched). `Apply Settings` copies the current settings clipboard onto every image in the collection non-destructively (applied automatically next time each image is opened). `Export...` batch-exports every image in the collection through a chosen preset in the background, reusing the batch-export job runner
